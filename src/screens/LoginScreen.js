...
export default function LoginScreen({ navigation, route }) {
  ...
  useEffect(() => {
    if (!route?.params?.passwordReset) return;
    setAlertModal({ show: true, message: "Password reset successfully. You can now log in using your new password.", extraAction: null });
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (response.requiresOtp) {
      navigation.navigate("otp", { email: response.email, purpose: "login" });
    } else {
                  ...
                  <View style={styles.inputContainer}>
                    ...
                    <View style={[styles.inputWrapper, fieldErrors.username && styles.inputWrapperError]}>
                      <TextInput
                        ...
                        onBlur={() => {
                          if (!username.trim()) {
                            setFieldErrors((current) => ({ ...current, username: "Username is required." }));
                          }
                        }}
